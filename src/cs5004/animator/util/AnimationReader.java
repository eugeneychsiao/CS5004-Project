...
public class AnimationReader {
   ...
   * <ul>
   * <li>cs5004.animator.model.Shape lines: the keyword "shape" followed by two identifiers (i.e.
  ...
  private static <Doc> void readShape(Scanner s, AnimationBuilder<Doc> builder) {
    ...
    if (s.hasNext()) {
      ...
      throw new IllegalStateException("cs5004.animator.model.Shape: "
                                      + "Expected a name, but no more input available");
    }
    if (s.hasNext()) {
      type = s.next();
    } else {
      throw new IllegalStateException("cs5004.animator.model.Shape: "
                                       + "Expected a type, but no more input available");
    }
